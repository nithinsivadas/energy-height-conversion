function [data] = altitudeInversionSIC( data_pfisr, ebin, DateNumBeg, DateNumEnd )
% 29-Jul-2016: Update
% 22-Aug-2016: Including production rates from the  SIC model

% Input:
% data_pfisr: struct [ ne, alt, q, time]
%             ne  - Electron Density [cm^-3]   
%             alt  - Altitude [km]
%             time - Time [matlab units]
%             q    - Production rates [cm^-3 s^-1]
...
q                = data_pfisr.q(zdim0,TimeBegIndex+1:TimeEndIndex);
...
% var_q   =var(q_1);
%Plotting A
A =generate_A(altitude(zdim0),Ebins',65.13,-147,datenum([2015 07 26 21 00 00]));
data.A=A;
% save('input_data.mat','ne_temp','altitude','t_temp','A');

data.q_PFISR = q; %Collecting production rate
...
%         [phi_new1(:,i),q_new(:,i),chi2(i),max_iter(i)] =mem_solve(q_1(:,i),A,beta,phi_new,sqrt(var_q(i)),5000,W);
        [phi_new1(:,i),q_new(:,i),chi2(i),max_iter(i)] =mem_solve(q_temp,A,beta,phi_new,sqrt(var_q(i)),5000,W);
...
data.time_ne  = t;
data.time_q   = t; % Due to differentiation the time for production rates is different
data.time_PFISR_E   = t;
