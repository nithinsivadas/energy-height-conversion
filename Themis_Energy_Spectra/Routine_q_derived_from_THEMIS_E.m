...
% time        = datenum([2001 02 11 05 18 20]); % Must be a scalar or string.
latitude    = 66.95; % Degrees.
longitude   = -50.94; % Degrees.
h           = 50:1:150; % km. Must be a scalar.

data1 = read_tplot_ascii('thd_peef_en_eflux_20080326.txt','thd_peef_en_eflux_v_20080326.txt');
data2 = read_tplot_ascii('thd_psef_en_eflux_20080326.txt','thd_psef_en_eflux_v_20080326.txt');
data  = combine_energies(data1,data2);
...
low_ebin      = 9;
E             = data.ebin(low_ebin:end);
dE            = diff(data.ebin(low_ebin-1:end));
flux          = data.E(:,low_ebin:end)*(diag(2*pi*E.^-1));
%% MSIS-90 Model

[D, T, F10_7_USED, AP_USED] = msis(data.time(1), latitude, longitude, h);
...
figure;
data_q.time=data2.time;
data_q.E   =q';
data_q.ebin=h;
% plot(q*10^6,h);
plot_eflux(data_q,0.5,'Production rate');
xlabel('Production Rate [cm^-^3s^-1]')
ylabel('Altitude [km]');
title ('2008-03-26 Production Rates Derived from  THEMIS');
caxis([0 6]);
grid on;
