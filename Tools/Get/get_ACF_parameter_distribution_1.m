...
outputNLR = zeros(4,nExperiments);
...
variance=zeros(2*nSamples,nPulseTrains);
...
for thisPulseTrain = 1:1:nPulseTrains
    ...
    variance(:,thisPulseTrain) = (diag(covR));
end

% Averaging the ACFs over the pulse trains
RArray2D = squeeze(mean(RArray3D,2));
% Calculating the corresponding std. deviation after averaging
stdDeviation = ((nPulseTrains.^-2).*sum(variance,2)).^0.5;

% Levenberg-Marquardt fitting
data = num2cell(RArray2D,1);
outputTemp = cellfun(@(x)run_nonlin_fitter(lag,x,stdDeviation,inputNLR0,options),data,'UniformOutput',false);
% Storing the fitted parameter values in the output variable
outputNLR = cell2mat(outputTemp); 

%%
% for thisExperiment = 1:1:nExperiments
%     data=RArray2D(:,thisExperiment);
...
%     % Storing the fitted parameter values in the output variable
%     outputNLR(:,thisExperiment) = outputTemp; 
% 
% end
