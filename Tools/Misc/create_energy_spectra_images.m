...
% Generate time array
    ...
    info.dasc = h5info(h5FileStr,'/DASC/');
    nDays = length(info.dasc.Groups);
    for iDay=1:nDays
        tempAddress(iDay,:) = [info.dasc.Groups(iDay).Name];
        tempFileSep = strsplit(tempAddress(iDay,:),'/');
        time.dasc.date(iDay) = datenum(tempFileSep(end),'yyyymmdd'); 
        time.dasc.h5Address(iDay,:)=[tempAddress(iDay,:),'/timeDASC'];
    end
%%
% Read time arrays
    ...
    if isempty(timeMaxStr)
        ...
        for iDay = 1:nDays
            time.dasc.value(:,iDay) = h5read(h5FileStr,time.dasc.h5Address(iDay,:));
            if floor(datenum(timeMinStr))==time.dasc.date(iDay)
                time.dasc.min.thisiDay = iDay;
                time.dasc.min.indx = find_time(time.dasc.value(:,iDay),timeMinStr);
            end
            if floor(datenum(timeMaxStr))==time.dasc.date(iDay)
                time.dasc.max.thisiDay = iDay;
                time.dasc.max.indx = find_time(time.dasc.value(:,iDay),timeMaxStr);
            end
        end
...
% Calculating primary time array  
    if time.pfisr.value(2)-time.pfisr.value(1)<=time.dasc.value(2,1)-time.dasc.value(1,1)
        timePrimary = time.pfisr.value;
    else
        dayArray = time.dasc.min.thisiDay:1:time.dasc.max.thisiDay;
        if length(dayArray)==1
        timePrimary = time.dasc.value(time.dasc.min.indx:time.dasc.max.indx,1);
        else
            for iDay = dayArray 
                if floor(datenum(timeMinStr))==time.dasc.date(iDay)
                    timePrimary = time.dasc.value(time.dasc.min.indx:end,iDay); 
                elseif time.dasc.date(iDay)>floor(datenum(timeMinStr)) && time.dasc.date(iDay)<floor(datenum(timeMaxStr))
                    n0 = length(timePrimary);
                    n1 = length(time.dasc.value(:,iDay));
                    timePrimary(n0+1:n0+1+n1) = time.dasc.value(:,iDay);
                elseif time.dasc.date(iDay)==floor(datenum(timeMaxStr))
                    n0 = length(timePrimary);
                    n1 = length(time.dasc.value(1:time.dasc.max.indx,iDay));
                    timePrimary(n0+1:n0+1+n1) = time.dasc.value(1:time.dasc.max.indx,iDay);
                end
            end
        end
    end
...
% Reading the important variables
    ...
    for iBeam = 1:1:nBeams
        pfisrLatitude(iBeam,1) = interp1(pfisrAltitude(iBeam,:),tempLatitude(iBeam,:),projectionAlt(1));
        pfisrLongitude(iBeam,1) = interp1(pfisrAltitude(iBeam,:),tempLongitude(iBeam,:),projectionAlt(1));  
    end
    ...
    for itime = 1:1:nTime
        ...
        dascDayIndx = dascDayIndxPrimary(itime);
        dascTimeIndx = find_time(time.dasc.value(:,dascDayIndx),thisTime);
        thisDascAddress = ['/DASC/',datestr(time.dasc.date(dascDayIndx),'yyyymmdd'),'/'];

        pfisrTime = time.pfisr.value(pfisrTimeIndx);
        dascTime = readh5_variable_at_time(h5FileStr,'timeDASC',thisDascAddress,dascTimeIndx);
    %     message = readh5_variable_at_time(h5FileStr,'message',thisDascAddress,dascTimeIndx);
        ...
        % Plotting
        figureHandle=figure;
        plot_2D_energy_slice_with_DASC_v2018(figureHandle,pfisrTime,...
...
function varValue=readh5_variable_at_time(h5FileStr,varStr,address,thisTimeIndx)
    ...
    matrix2DwithTimeVars = {'ASI','az_new','el_new','lat','lon','timeDASC',...
        'MSE','maxIter','time'};
    matrix3DwithTimeVars = {'dEnergyFlux','energyFlux','flux','qError',...
        'qInput','qInverted','Ne','dNeFrac',};
    matrix2D = {'azCalData','elCalData','sensorloc','A','alt','energyBin',...
        'projectionAlt'};
...
function plot_2D_energy_slice_with_DASC_v2018(figureHandle,pfisrTime,...
    diffEnergyFlux, pfisrLatitude, pfisrLongitude,...
        zEnergyBin, energySlice, eFluxLim, opticalData, dascTime,...
        dascLatitude, dascLongitude,...
...
if isempty(eFluxLim)
    ...
    imageName = strcat('figure_',datestr(dascTime,'HH_MM_SS'));
