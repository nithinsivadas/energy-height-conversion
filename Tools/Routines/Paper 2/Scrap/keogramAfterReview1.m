%% Routine Keogram
% Two goals:
% 1. Estimate the latitudinal distance between the current-sheet scattered
% energies


%% Section1: Estimating the latitudinal distance between the current-sheet scattered energies
clear all;
...
sensorLoc(3) = 0.689;
meridian = get_magnetic_meridian(sensorLoc,datenum('26 Mar 2008 11:00'),parallels,110);        
%% PFISR Keogram
...
[pfisrImage10,pfisrLat10,pfisrLon10,projAlt10] = get_pfisr_energy_slice(pfisrImage,pfisrData,10,100); 
[pfisrKeo10,pfisrPar10] = create_keogram(pfisrImage10,pfisrLat10,pfisrLon10,'meridian',meridian,'latPixelNum',6);
%%
...
hold on;
h100 = plot(pfisrData.time(timeMinIndx:timeMaxIndx-35),peakLatitude100(timeMinIndx:timeMaxIndx-35),'g','LineWidth',1); %100
...
% h10 = plot(pfisrData.time(timeMinIndx+35:timeMaxIndx-20),peakLatitude10(timeMinIndx+35:timeMaxIndx-20),'c','LineWidth',1); %30
label_time_axis(dascData.time,false,0.25,timeMinStr,timeMaxStr);
...
ylabel({'Peak Energies','lat [N^\circ]'});

% Figure 4b 100 keV Electron Energy Flux
q(3).select();
plot_pfisr_energy_keogram(pfisrKeo100,pfisrPar100,pfisrData,true,timeMinStr,timeMaxStr,'[eV/m^2 sr s eV]');
ylabel({'e^- Energy Flux','100 keV','lat [N^\circ]'});

q(2).select();
plot(pfisrData.time(timeMinIndx:timeMaxIndx),peakLatitude30(timeMinIndx:timeMaxIndx)-peakLatitude100(timeMinIndx:timeMaxIndx),'r','LineWidth',1);
ylabel({'\Delta peak energies','30-100 keV','[keV]'});
label_time_axis(dascData.time,false,0.25,timeMinStr,timeMaxStr);
legend('\Delta Lat ~ 0.25^\circ');

%%
figure;

label_time_axis(dascData.time,true,0.25,timeMinStr,timeMaxStr);
