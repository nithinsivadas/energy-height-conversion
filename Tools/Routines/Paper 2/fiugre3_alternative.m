...
%%
smoothFactor = 10;
kext = find_irbem_magFieldModelNo('TS01storm');
...
trappedFlux =padData.thd.trappedDiffEfluxLi;
trappedFlux(trappedFlux<0) = nan;
lcFlux = padData.thd.lcDiffEfluxLi;
lcFlux(lcFlux<1) = nan;
zValue1 = trappedFlux'./lcFlux' - 1;
...
ylabel({'Themis-D','Electron anisotropy','[keV]'});
caxis(cLim);
cb2=colorbar('eastoutside','Ticks',[-1 0 +1],'TickLabels',{'-1','0','+1'});
cb2.Position(1) = 0.9;
cb2.Position(3) = 0.01;
cb2.Label.String = {'\phi_\perp/\phi_|_| - 1'};
...
% % plot(time,cumEnergyBin(medianIndx),'c','LineWidth',1);
% 
...
% plot(time,cumEnergyBinLow(medianIndx),'m','LineWidth',1);

% % Calculate the peak values of energy flux per electron energy, where anisotropy < -1
% anisotropy = zValue1;
% energyBin = yAxis; 
% anisotropyFilter = zeros(size(anisotropy));
% anisotropyCutOff = -0.95;
% anisotropyFilter(anisotropy<=anisotropyCutOff) = 1;
% zValueTemp = zValue.*anisotropyFilter;
% zValueTemp(isnan(zValueTemp)) = 0;
% 
% for iE = 32:1:length(energyBin)
%     f = fit(time,zValueTemp(iE,:)','gauss1');
%     peakEnergyFluxTime(iE,1) = f.b1;
% end
% hold on;
% plot(peakEnergyFluxTime,energyBin,'*c','LineWidth',1);
...
zValue(zValue<0) = 10^3;
trappedFlux =padData.the.trappedDiffEfluxLi;
trappedFlux(trappedFlux<0) = nan;
lcFlux = padData.the.lcDiffEfluxLi;
lcFlux(lcFlux<1) = nan;
zValue1 = trappedFlux'./lcFlux' - 1;
midEBin = yAxis(find_altitude(yAxis,midEnergy));
...
set(gca,'YTick',[1,10,100,300],'YTickLabel',{'1','10','100','300'},...
    'YLim',[1,1000],'YScale','log');
ylabel({'Themis-E','Electron anisotropy','[keV]'});
caxis(cLim);
cb3=colorbar('eastoutside','Ticks',[-1 0 1],'TickLabels',{'-1','0','+1'});
cb3.Position(1) = 0.9;
cb3.Position(3) = 0.01;
cb3.Label.String = {'\phi_\perp/\phi_|_| - 1'};
