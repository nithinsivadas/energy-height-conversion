...
Y = omni.Fsml(omni.time);
X = omni.Fekl(omni.time).*10^-3;
Y(Y==999999)=nan;
...
%% E_m|SML
Ey = linspace(-4500,15,100);
[yindx, Ey] = discretize(Y,Ey);
for i=1:max(yindx)
    XgY(i) = nanmean(X(yindx==i));
    NySamples(i) = sum(yindx==i & ~isnan(X));
    SEMy(i) = nanstd(X(yindx==i))./sqrt(NySamples(i));
    tsy(i,:) = tinv([0.025 0.975],NySamples(i)-1);
    CIy(i,:) = XgY(i) + tsy(i,:)*SEMy(i);
    YBins(i) = 0.5*(Ey(i)+Ey(i+1));
end
