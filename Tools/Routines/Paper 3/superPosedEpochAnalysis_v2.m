...
%% kstest
for i =1:180
    multi = trapz(minutes(T.timeArrayRelOnset(1,1+i:10+i)),interp_nans(T.EArray(multiSubstorm,1+i:10+i)')',2);
    single = trapz(minutes(T.timeArrayRelOnset(1,150:160)),interp_nans(T.EArray(singleSubstorm,150:160)')',2);
    [h1(i),p1(i),ks2test1(i)] = kstest2(single,multi,'alpha',0.01);
end

%% kstest
clearvars h1 p1 ks2test1
for i =1:360
    multi = T.EArray(multiSubstorm,i);
    single = T.EArray(singleSubstorm,i);
    [h1(i),p1(i),ks2test1(i)] = kstest2(single,multi,'alpha',0.01);
end
figure; 
yyaxis left
...
plot(-180:179,p1,'-r');
ylim([-0.1 1.1]);
% need to check the definition of p value 
yyaxis right
plot(-180:179,ks2test1);
xlabel('Minutes');
legend('Hypothesis: Multi-onset and single-onset are the same distributions','probability that is not the case','D^* (distance between the CDFs)','Location','northoutside');
...
title('Post-Onset');
ylabel('Count Density');
%% kstest2 - If h2=1, -> that the two distributions are from different continuous pdfs

% [h1,p1,ks2test1] = kstest2(trapz(singleStorm.EArray(2,:,growthTIndex),3),trapz(multiStorm.EArray(2,:,growthTIndex),3));
for i=1:1:360
[h2(i),p2(i),ks2test2(i)] = kstest2(trapz(singleStorm.EArray(2,:,i),3),trapz(multiStorm.EArray(2,:,i),3),'Alpha',0.01);
end
figure; 
yyaxis left
plot(h2); 
