...
%% Not working as expected - do not know why yet. 
clear all;
%% Initializing
...
timeArr = datenum('26-Mar-2008 11:15');
tic
...
viewArr = [az,el];
storeImageDir = 'G:\My Drive\Research\Projects\Paper 2\Data\Figures\Cartoon\';
...
%     plot3(thaGSM(:,1)',thaGSM(:,2)',thaGSM(:,3)','k','LineWidth',3);
...
%     ax3.CLim = [0 1.4*10^4];
    ...
    plot3(thdGSM(:,1)',thdGSM(:,2)',thdGSM(:,3)','k','LineWidth',3);
...
%         'FaceColor','none','EdgeColor','interp','LineWidth',3);
    ...
    plot3(thdGSM(thisTIndx,1),thdGSM(thisTIndx,2),thdGSM(thisTIndx,3),'*b');
...
%         'FaceColor','none','EdgeColor','interp','LineWidth',3);
    plot3(theGSM(:,1)',theGSM(:,2)',theGSM(:,3)','k','LineWidth',3);

    hold on;
  
    thisTIndx = find_time(theTime,thisTime);
    plot3(theGSM(thisTIndx,1),theGSM(thisTIndx,2),theGSM(thisTIndx,3),'*c');     
...
ax1.YLim =[min([ ax4.YLim(1) ax5.YLim(1)]) max([ax4.YLim(2),ax5.YLim(2)])]; % Important
